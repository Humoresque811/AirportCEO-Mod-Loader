...
internal class Installer
{
    ...
    public void TryFindPath()
    {
        ...
        {
            Console.WriteLine("Not a windows machine... This won't work sadly");
    ...
    public void TryMove()
    {
        ...
        try
        {
            ...
            {
                string newPath = Path.Combine(ExePath, Path.GetFileName(directories[i]));
                ...
                Console.WriteLine($"Moving {Path.GetDirectoryName(directories[i])}");
                Directory.Move(directories[i], newPath);
            }

            string[] files = Directory.GetFiles(winReasourcePath);
            for (int i = 0; i < files.Length; i++)
            {
                string newPath = Path.Combine(ExePath, Path.GetFileName(files[i]));
                ...
                Console.WriteLine($"Moving {Path.GetFileName(files[i])}");
